...
"""Rosbag accuracy test: replays scan+odom at original timing, validates PGORust outputs."""
...
class TestPGORosbag:
    """Validate PGORust native module accuracy against OG nav stack recording."""

    def test_pgo_corrected_odometry(self) -> None:
        """Feed scan + odom at original timing and validate PGORust outputs.

        Checks:
        - PGORust produces corrected odometry messages
        ...
        logger.info(f"\n{'=' * 60}")
        logger.info("PGORust NATIVE ROSBAG DEVIATION SCORE")
        ...
        assert corrected_count > 0, "PGORust produced no corrected odometry"
        assert global_map_count > 0, "PGORust produced no global map messages"
        ...
        # PGORust shouldn't collapse the trajectory to a point or explode it.
