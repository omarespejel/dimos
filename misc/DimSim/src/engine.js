...
// ── Apartment-decompose pipeline ─────────────────────────────────────────────
// Drives Three.js's own GLTFExporter against every state of every asset and
// POSTs the .glb bytes to the bridge (cli/bridge/server.ts → /export-asset).
// One GLB per (asset, state) pair, so multi-state objects (fridge open/closed,
// cabinet drawers, gas-range knobs) keep their state machine when the apartment
// reloads from GLBs. The manifest carries asset-level metadata — transform,
// _shapePivotCenter, actions, pickable, bumpable, currentStateId — so the
// loader rebuilds full behavior, not just visuals.
//
// Usage from devtools after apartment loads:
//   await dimsim.exportApartmentAssets()              // all assets, all states
//   await dimsim.exportApartmentAssets({ limit: 1 })  // first asset — smoke test
//   await dimsim.exportApartmentAsset('id-or-title')  // one by name/id
function _slugifyForGlb(s) {
...
async function _waitTexturesLoaded(root) {
  const texSlots = [
    "map", "normalMap", "roughnessMap", "metalnessMap", "aoMap", "emissiveMap",
    "bumpMap", "displacementMap", "specularColorMap", "clearcoatMap",
  ...
  root.traverse((node) => {
    ...
    for (const m of mats) {
      for (const key of texSlots) if (m && m[key]) textures.add(m[key]);
    }
  });
  await Promise.all([...textures].map((tex) => new Promise((resolve) => {
    const ready = () => !!(tex.image && (tex.image.complete !== false) && (tex.image.width || tex.image.naturalWidth));
    if (ready()) return resolve();
    const start = Date.now();
    const tick = () => {
      if (ready() || Date.now() - start > 10000) return resolve();
      setTimeout(tick, 50);
    };
    tick();
  })));
}
async function _exportAssetAllStates(asset, exporter, assetSlugTaken, libCache) {
  if (!asset?.states?.length) return null;

  // Storage dedup: if this asset shares libraryAssetId with one we've already
  // exported, reuse those state GLB files. Manifest still gets its own entry
  // per placed instance (each carries instance-specific transform, action IDs,
  // and state IDs), but the `file` paths point to the shared library asset.
  const libId = asset.libraryAssetId;
  if (libId && libCache && libCache.has(libId)) {
    const cached = libCache.get(libId);
    const stateEntries = asset.states.map((s, i) => ({
      id: s.id,
      name: s.name || s.id,
      // Match by state index (instances of the same library have parallel state arrays).
      file: cached.stateFiles[i] || cached.stateFiles[0],
    }));
    return {
      id: asset.id,
      title: asset.title || null,
      transform: asset.transform || null,
      currentStateId: asset.currentStateId || stateEntries[0].id,
      _shapePivotCenter: asset._shapePivotCenter || null,
      pickable: asset.pickable === true,
      bumpable: asset.bumpable === true,
      bumpResponse: asset.bumpResponse,
      bumpDamping: asset.bumpDamping,
      castShadow: asset.castShadow === true,
      receiveShadow: asset.receiveShadow === true,
      actions: Array.isArray(asset.actions) ? asset.actions : [],
      states: stateEntries,
      _bytes: 0,           // already counted on the first export
      _libRef: libId,
    };
  }

  // Stable asset slug (collision-free across the run)
  let assetSlug = _slugifyForGlb(asset.title || asset.id);
  let n = 1;
  while (assetSlugTaken.has(assetSlug)) { n += 1; assetSlug = `${_slugifyForGlb(asset.title || asset.id)}-${n}`; }
  assetSlugTaken.add(assetSlug);

  // Use the same pivot the engine baked in so every state lines up with the
  // asset's transform.position (which is the pivot in world space).
  const pivot = asset._shapePivotCenter
    ? new THREE.Vector3(asset._shapePivotCenter.x, asset._shapePivotCenter.y, asset._shapePivotCenter.z)
    : null;

  const stateEntries = [];
  let totalBytes = 0;
  const stateSlugTaken = new Set();

  for (const state of asset.states) {
    let stateSlug = _slugifyForGlb(state.id || state.name || "state");
    let k = 1;
    while (stateSlugTaken.has(stateSlug)) { k += 1; stateSlug = `${_slugifyForGlb(state.id || state.name || "state")}-${k}`; }
    stateSlugTaken.add(stateSlug);
    const file = `${assetSlug}/${stateSlug}.glb`;

    try {
      // Build a fresh state root via the same path instantiateAsset uses,
      // independent of which state is currently visible in the scene.
      const root = buildShapeStateRoot(state, asset.id, pivot);
      // Local-space GLB: zero the root so children sit relative to the asset
      // origin (apartment.transform will be applied on load).
      root.position.set(0, 0, 0);
      root.rotation.set(0, 0, 0);
      root.scale.set(1, 1, 1);
      root.updateMatrixWorld(true);

      // TextureLoader.load is async; wait for images to actually decode before
      // GLTFExporter inspects them. Otherwise "No valid image data" on any
      // primitive whose textureDataUrl fetch hasn't completed yet.
      await _waitTexturesLoaded(root);

      const buf = await exporter.parseAsync(root, { binary: true, embedImages: true });
      disposeShapeStateRoot(root);

      const res = await fetch(`/export-asset?name=${encodeURIComponent(file)}`, {
        method: "POST", body: buf, headers: { "content-type": "model/gltf-binary" },
      });
      if (!res.ok) { console.warn(`[export] ${file} failed: ${res.status}`); continue; }
      totalBytes += buf.byteLength;
      stateEntries.push({ id: state.id, name: state.name || state.id, file });
    } catch (e) {
      console.error(`[export] ${asset.title || asset.id} state "${state.name || state.id}" failed`, e);
    }
  }

  if (stateEntries.length === 0) return null;

  // Record this library asset's state files so future placed instances of
  // the same library reference these files instead of re-exporting bytes.
  if (libId && libCache) {
    libCache.set(libId, { stateFiles: stateEntries.map((e) => e.file) });
  }
...
window.dimsim = window.dimsim || {};
window.dimsim.exportApartmentAssets = async function ({ limit = null, writeManifest = true } = {}) {
  const { GLTFExporter } = await import("three/examples/jsm/exporters/GLTFExporter.js");
  const exporter = new GLTFExporter();
  const list = (Array.isArray(assets) ? assets : []).filter((a) => a && a.id && a.states?.length);
  const sliced = limit != null ? list.slice(0, limit) : list;
  console.log(`[export] starting: ${sliced.length} asset(s), all states`);
  const manifest = [];
  const taken = new Set();
  const libCache = new Map();  // libraryAssetId → { stateFiles }
  let idx = 0;
  let dedupedCount = 0;
  ...
  for (const a of sliced) {
    idx += 1;
    try {
      const entry = await _exportAssetAllStates(a, exporter, taken, libCache);
      if (entry) {
        const { _bytes, _libRef, ...keep } = entry;
        manifest.push(keep);
        if (_libRef) {
          dedupedCount += 1;
          console.log(`[export] ${idx}/${sliced.length} ${a.title || a.id}  reused library ${_libRef}`);
        } else {
          totalBytesWritten += _bytes;
          console.log(`[export] ${idx}/${sliced.length} ${a.title || a.id}  ${entry.states.length} state(s), ${(_bytes/1024).toFixed(1)} KB`);
        }
      } else {
        console.warn(`[export] ${idx}/${sliced.length} skipped: ${a.id}`);
      }
    } catch (e) {
      console.error(`[export] ${idx}/${sliced.length} ${a.title || a.id} failed`, e);
    }
  }
  console.log(`[export] ${manifest.length} entries, ${dedupedCount} deduped (library refs), ${(totalBytesWritten/1024/1024).toFixed(1)} MB written`);
  if (writeManifest && manifest.length > 0) {
    await fetch("/export-manifest", {
      method: "POST", body: JSON.stringify(manifest, null, 2),
      headers: { "content-type": "application/json" },
    });
  }
  console.log(`[export] done: ${manifest.length}/${sliced.length}`);
  return manifest;
};
window.dimsim.exportApartmentAsset = async function (idOrTitle) {
  const list = Array.isArray(assets) ? assets : [];
  const needle = String(idOrTitle).toLowerCase();
  const a = list.find((x) => x.id === idOrTitle) ||
            list.find((x) => (x.title || "").toLowerCase() === needle);
  if (!a) { console.error(`[export] no asset matching "${idOrTitle}"`); return null; }
  const { GLTFExporter } = await import("three/examples/jsm/exporters/GLTFExporter.js");
  const exporter = new GLTFExporter();
  const entry = await _exportAssetAllStates(a, exporter, new Set());
  if (entry) {
    const { _bytes, ...rest } = entry;
    console.log(`[export] ${a.title || a.id}: ${entry.states.length} state(s), ${(_bytes/1024).toFixed(1)} KB`);
    return rest;
  }
  return null;
};
// Bakes the apartment's static-structure primitives (walls, floor, ceiling,
// fixtures — everything in data/structure.js) into a single GLB on disk via
// the bridge's /export-structure endpoint. Used once during the conversion
// from data-driven primitives to a pure-Three.js scene module.
window.dimsim.exportStructure = async function () {
  const { GLTFExporter } = await import("three/examples/jsm/exporters/GLTFExporter.js");
  const exporter = new GLTFExporter();
  const root = new THREE.Group();
  root.name = "apartment-structure";
  for (const p of primitives) {
    const geom = createPrimitiveGeometry(p.type, p.dimensions || {});
    const mat = createPrimitiveMaterial(p.material || {});
    const mesh = new THREE.Mesh(geom, mat);
    mesh.name = p.id || p.name || "primitive";
    const tr = p.transform || {};
    if (tr.position) mesh.position.set(tr.position.x ?? 0, tr.position.y ?? 0, tr.position.z ?? 0);
    if (tr.rotation) mesh.rotation.set(tr.rotation.x ?? 0, tr.rotation.y ?? 0, tr.rotation.z ?? 0);
    if (tr.scale) mesh.scale.set(tr.scale.x ?? 1, tr.scale.y ?? 1, tr.scale.z ?? 1);
  ...
  await _waitTexturesLoaded(root);
  const buf = await exporter.parseAsync(root, { binary: true, embedImages: true });
  const res = await fetch("/export-structure", {
    method: "POST", body: buf, headers: { "content-type": "model/gltf-binary" },
  });
  if (!res.ok) throw new Error(`export-structure failed: ${res.status}`);
  console.log(`[export-structure] wrote ${(buf.byteLength/1024).toFixed(1)} KB (${primitives.length} primitives)`);
  root.traverse((o) => {
    if (!o.isMesh) return;
    o.geometry?.dispose?.();
  ...
  return { bytes: buf.byteLength, count: primitives.length };
};
