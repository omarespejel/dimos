...
"""
Spins up a blueprint with PGORust, the KITTI-360 playback module, and a
TopicCounter module that subscribes to every PGORust output. Reports per-topic
...
class TopicCounterModule(Module):
    """Subscribes to every PGORust output stream and counts arrivals per topic."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PGORust liveness probe via DimOS modules")
    ...
    if counts.get("pose_graph", 0) == 0:
        print("  ⚠ no pose graph — PGORust never promoted a keyframe. Check --key_pose_delta_*.")
        ...
        print("  ✓ all topics firing — PGORust is alive end-to-end.")
