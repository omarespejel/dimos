...
This test runs PGORust twice with the same input via the DimOS Module +
...
# Drain after the playback module reports finished, so PGORust can flush
...
# After the first scan goes out, wait this long for PGORust to emit anything
...
def _trajectory_with_drift(
    ...
    # monotonic-ts assumption in PGORust's on_registered_scan.
    timestamp = 1.0
    time_step = 0.5
...
class SyntheticDriftPlaybackModule(Module):
    ...
    # Subscribed only so we can detect when PGORust has come up and processed the
    # first scan — see _run_playback's "wait for PGORust ack" gate.
    ...
    async def _run_playback(self) -> None:
        ...
        try:
            ...
            for frame_index, row in enumerate(self.config.trajectory):
                ...
                if frame_index == 0:
                    # Wait for PGORust to publish anything (corrected_odometry)
                    # before sending the rest of the trajectory, so we don't
                    # race PGORust's startup.
                    try:
...
def _run_pgo(
    ...
    """Build the blueprint, run the synthetic trajectory through PGORust, return loop count."""
