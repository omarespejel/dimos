...
async def _handle_tools_call(
    ...
    if uses:
        conflict = cap_registry.acquire(uses, holder=name)
        if conflict is not None:
            ...
            return _jsonrpc_result_text(
                req_id,
                f"Cannot start '{name}': capability '{cap}' is held by '{holder}'. "
                f"Call the appropriate stop tool first, then retry.",
            )
    ...
    try:
        ...
        if caps_held:
            cap_registry.release_by_holder(name)
...
def _fan_out_to_sse_queues(msg: dict[str, Any]) -> None:
    ...
    if msg.get("method") == tool_stream.TOOL_STREAM_STOPPED_METHOD:
        ...
        if tool_name:
            released = app.state.cap_registry.release_by_holder(tool_name)
            if released:
                logger.info(
